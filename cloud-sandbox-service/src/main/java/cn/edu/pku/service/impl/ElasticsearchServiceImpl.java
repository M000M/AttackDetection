...
public class ElasticsearchServiceImpl implements ElasticsearchService {
    ...
    public long getTotal() {
        ...
        int lastPosition = positionMapper.lastPosition();
        if (res > lastPosition) {
            positionMapper.updatePosition(res);
            parseIncrementalLogs(lastPosition, (int)res);
        }
        return res;
    }

    private void parseIncrementalLogs(int lastPosition, int newPosition) {
        ...
        for (Object obj: logs) {
            ...
            rabbitTemplate.convertAndSend("ip address", "", obj1.getString("log")); //发送到消息队列
        }
    }
